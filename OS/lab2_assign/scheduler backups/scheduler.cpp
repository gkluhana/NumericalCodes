...
void Simulation(list& event_queue, fileRead* randfile, Process* summary[],char sched_type, int timequantum);
int myrandom(int burst, fileRead *randfile);
void printSummary(Process* summary[], int num_processes);
...
int main(int argc, char** argv) {
    ...
    //Simulation
    Simulation(Event_Queue,randfile, Summary,sched_type, timequantum);
    // Print output
    printSummary(Summary,num_processes);
}
//Main Simulation
void Simulation(list& Event_Queue,fileRead* randfile, Process* summary[], char sched_type, int timequantum){
    ...
    while((evt)=Event_Queue.get_event()){
        ...
        switch(evt -> transition()){ // which state to transition to?
            case TRANS_TO_READY:
                if (proc->getPS()==STATE_BLOCKED){proc->reset_PRIO_dynamic();proc->make_active();}
                //add to Run Queue
...
void printSummary(Process* Summary[], int num_processes){
    ...
    io_percentage = (sum_io/end_time)*100;
