...
frame_t* CLOCK::select_victim_frame(vector<Process> &plist){
 	...
 	while(true){
 	if(!plist[(frame_table[hand]).pid].ptable[(frame_table[hand]).vaddr].referenced){
	 	
	 	frame_t *tempframe = &frame_table[hand];
	 	hand++;
		...
		plist[(frame_table[hand]).pid].ptable[(frame_table[hand]).vaddr].referenced=0;
		hand++;
	 	if(hand>=frame_table.size()){
	 		hand=0;
	 	}
	...
	while (getline(inputfile-> inFile,inputfile->line)){
		...
		if (pte->created && !pte->present){
			...
			if(plist[newframe->pid].ptable[newframe->vaddr].modified){
				...
				if(!plist[newframe->pid].ptable[newframe->vaddr].paged_out){
				plist[newframe->pid].ptable[newframe->vaddr].star=1;}
			}
			}
			...
			current_process->ptable[vpage].star=0;
			cost = cost + 400;
		}
		//Execute Instruction
			if(*inst_type=='w'){
